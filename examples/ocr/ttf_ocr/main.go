...
// train - train network
func train(net *cnns.WholeNet, data map[string][]*tensor.Tensor, chars map[int]string) error {
	st := time.Now()
	var err error
	...
	for k, v := range data {
		...
		for mat := range v {
			var temp Trainer
			temp.Desired = tensor.NewTensor(len(chars), 1, 1)
			var target [][][]float64
			target = append(target, [][]float64{})
			target[0] = append(target[0], []float64{})
			for range chars {
				target[0][0] = append(target[0][0], 0.0)
			}
			target[0][0][intK] = 1.0
...
func testTrained(net *cnns.WholeNet, data map[string][]*tensor.Tensor, chars map[int]string) error {
	var err error
	var testers []Trainer
	for k, v := range data {
		intK, err := strconv.Atoi(k)
...
func generatePNGs() (map[int]string, error) {
	...
	for i := range dict {
		...
		dc.SetRGB(0, 0, 0)
		dc.Scale(1.0, 1.0)
		err := dc.LoadFontFace("./lucidagrande.ttf", 38)
		if err != nil {
			return nil, err
		}
